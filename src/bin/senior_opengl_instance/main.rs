...
use glium::{glutin::{window::CursorGrabMode}, PolygonMode, VertexBuffer, IndexBuffer, index::PrimitiveType, uniforms::UniformValue, vertex::EmptyInstanceAttributes};

use rust_opengl_learn::{camera::{CameraController}, uniforms::DynamicUniforms, keyboard, create_program, objectsv2::{RawVertexP2C}};
...
fn main() {
    ...
    let program = create_program(
        "src/bin/senior_opengl_instance/rect.vert", 
        "src/bin/senior_opengl_instance/rect.frag", 
        &display);
    ...
    for i in 0..10 {
        ...
        for j in 0..10 {
            let x = (j * 2 - 10) as f32;
            // [x / 10.0 + offset, y / 10.0 + offset]
            uniforms.add_str_key_value(format!("offsets[{}]", index).as_str(), UniformValue::Vec2([x / 10.0 + offset, y / 10.0 + offset]));
            index += 1;
        }
    }
    ...
    event_loop.run(move |event, _, control_flow| {
        ...
        *control_flow = event_loop::ControlFlow::WaitUntil(next_frame_time);
        ...
        let instances = EmptyInstanceAttributes { len: index };
        // 循环渲染模型
        target.draw((&vertex_buffer, instances), &index_buffer, &program, &uniforms, &draw_parameters).unwrap();

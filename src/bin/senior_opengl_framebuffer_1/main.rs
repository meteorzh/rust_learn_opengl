...
use glium::{glutin::{event::{KeyboardInput, VirtualKeyCode, ElementState}, window::CursorGrabMode}, backend::Facade, framebuffer, texture, draw_parameters, uniforms::{UniformValue, self}};
...
fn main() {
    ...
    // 物体着色器程序
    let obj_program = create_program("src/bin/senior_opengl_framebuffer_1/obj_shader_0.vert", "src/bin/senior_opengl_framebuffer_1/obj_shader_0.frag", &display);
    // 渲染帧缓冲程序
    ...
    event_loop.run(move |event, _, control_flow| {
        ...
        *control_flow = event_loop::ControlFlow::WaitUntil(next_frame_time);
        ...
        let render_buffer = framebuffer::DepthStencilRenderBuffer::new(&display, texture::DepthStencilFormat::I24I8, dimensions.0, dimensions.1).unwrap();
        let mut target = framebuffer::SimpleFrameBuffer::with_depth_stencil_buffer(&display, &texture, &render_buffer).unwrap();
        ...
        box_uniforms.add_str_key("projection", &projection_matrix);
        box_uniforms.add_str_key("viewPos", &camera_position);
        ...
        let texture_value = UniformValue::SrgbTexture2d(&texture, Some(uniforms::SamplerBehavior {
            wrap_function: (uniforms::SamplerWrapFunction::Clamp, uniforms::SamplerWrapFunction::Clamp, uniforms::SamplerWrapFunction::Clamp),
            ..Default::default()
        }));
        screen_uniforms.add_str_key_value("texture1", texture_value);
        frame.draw(&screen.vertex_buffer, &screen.index_buffer, &screen_program, &screen_uniforms, &Default::default()).unwrap();
        // 其它的方式填充frame
        // texture.as_surface().fill(&frame, glium::uniforms::MagnifySamplerFilter::Nearest);
        // target.blit_whole_color_to(&frame, &glium::BlitTarget {
        //     left: 0,
        //     bottom: 0,

...
fn main() {
    ...
    // 物体着色器程序
    let obj_program = create_program("src/bin/senior_opengl_cubemap_exercise/object.vert", "src/bin/senior_opengl_cubemap_exercise/object.frag", &display);
    // skybox着色器程序
    let skybox_program = create_program("src/bin/senior_opengl_cubemap_exercise/skybox.vert", "src/bin/senior_opengl_cubemap_exercise/skybox.frag", &display);

    let skybox = Cube::new_skybox("skybox", 2.0, &display);
    let models = load_wavefront_obj_as_models(&display, "src/nanosuit_reflection/", "nanosuit.obj");
    ...
    event_loop.run(move |event, _, control_flow| {
        ...
        *control_flow = event_loop::ControlFlow::WaitUntil(next_frame_time);
        ...
        box_uniforms.add_str_key("material.skybox", &skybox_texture);
        // 循环渲染模型
        for model in models.iter() {
            if let Some(material) = &model.material {
                material.add_to_uniforms("material", &mut box_uniforms);
            }
        ...
        box_uniforms.remove("material.skybox");

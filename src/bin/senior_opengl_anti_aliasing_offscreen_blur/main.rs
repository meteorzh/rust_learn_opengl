...
use glium::{glutin::{window::CursorGrabMode, dpi::LogicalSize}, framebuffer::{DepthStencilRenderBuffer, SimpleFrameBuffer}, texture::DepthStencilFormat, backend::Facade};

use rust_opengl_learn::{camera::{Camera, CameraController}, uniforms::DynamicUniforms, objects::{Cube, Plane}, create_program, keyboard::handle_keyboard_input};
...
fn main() {
    ...
    // 物体着色器程序
    let program = create_program("src/bin/senior_opengl_anti_aliasing_offscreen_blur/cube.vert", "src/bin/senior_opengl_anti_aliasing_offscreen_blur/cube.frag", &display);
    let blur_program = create_program("src/bin/senior_opengl_anti_aliasing_offscreen_blur/blur.vert", "src/bin/senior_opengl_anti_aliasing_offscreen_blur/blur.frag", &display);
    ...
    event_loop.run(move |event, _, control_flow| {
        ...
        *control_flow = event_loop::ControlFlow::WaitUntil(next_frame_time);
        ...
        // 创建多重采样framebuffer并绘制
        let samples = 4;
        let dimensions = display.get_context().get_framebuffer_dimensions();
        let texture = glium::texture::SrgbTexture2dMultisample::empty(&display, dimensions.0, dimensions.1, samples).unwrap();
        let render_buffer = DepthStencilRenderBuffer::new_multisample(&display, DepthStencilFormat::I24I8, dimensions.0, dimensions.1, samples).unwrap();
        let mut target = SimpleFrameBuffer::with_depth_stencil_buffer(&display, &texture, &render_buffer).unwrap();
        ...
        // 将多重采样缓冲还原到中介FBO上
        let texture = glium::texture::SrgbTexture2d::empty(&display, dimensions.0, dimensions.1).unwrap();
        let render_buffer = DepthStencilRenderBuffer::new(&display, DepthStencilFormat::I24I8, dimensions.0, dimensions.1).unwrap();
        let mut rollback = SimpleFrameBuffer::with_depth_stencil_buffer(&display, &texture, &render_buffer).unwrap();
        rollback.clear_color_and_depth((0.05, 0.05, 0.05, 1.0), 1.0);

        target.blit_whole_color_to(&rollback, &glium::BlitTarget {
            left: 0,
            bottom: 0,
        ...
        uniforms.add_str_key("texture", &texture);
        frame.draw(&screen.vertex_buffer, &screen.index_buffer, &blur_program, &uniforms, &Default::default()).unwrap();

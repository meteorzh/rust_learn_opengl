...
use glium::{draw_parameters::{Depth}, glutin::{window::CursorGrabMode}, PolygonMode, uniforms::UniformValue, DepthTest};

use rand::{rngs::StdRng, SeedableRng, Rng};
use rust_opengl_learn::{camera::{CameraController, Camera}, uniforms::DynamicUniforms, keyboard, create_program_vgf, load_wavefront_obj_as_models, create_program};
...
fn main() {
    ...
    let rock_models = {
        ...
        for i in 0..amount {
            ...
            let displacement: f32 = rng.gen_range(-offset..=offset);
            let x = angle.sin() * radius + displacement;
            let y = displacement * 0.4;
            let z = angle.cos() * radius + displacement;
            let mut model = Matrix4::from_translation(Vector3::new(x, y, z));

            // 2. 缩放：在 0.05 和 0.25f 之间缩放
            model = Matrix4::from_scale(rng.gen_range(0.05..=0.25)) * model;

            // 3. 旋转：绕着一个（半）随机选择的旋转轴向量进行随机的旋转
            model = Matrix4::from_axis_angle(rotate_dir, Deg(rng.gen_range(0..360) as f32)) * model;
            rock_models.push(Into::<[[f32; 4]; 4]>::into(model));
        }
        rock_models
    };
    ...
    event_loop.run(move |event, _, control_flow| {
        ...
        *control_flow = event_loop::ControlFlow::WaitUntil(next_frame_time);
        ...
        for model in models.iter() {
            ...
            target.draw(&model.vertex_buffer, &model.index_buffer, &program, &uniforms, &draw_parameters).unwrap();
        }
        // 绘制小行星带
        for model in rocks.iter() {
            ...
            for rock_model in &rock_models {
                uniforms.add_str_key("model", rock_model);
                target.draw(&model.vertex_buffer, &model.index_buffer, &program, &uniforms, &draw_parameters).unwrap();
            }
        }

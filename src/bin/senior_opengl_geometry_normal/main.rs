...
fn main() {
    ...
    let program = create_program(
        "src/bin/senior_opengl_geometry_normal/model.vert", 
        "src/bin/senior_opengl_geometry_normal/model.frag", 
        &display);
    let normal_program = create_program_vgf(
        "src/bin/senior_opengl_geometry_normal/normal.vert", 
        "src/bin/senior_opengl_geometry_normal/normal.geom", 
        "src/bin/senior_opengl_geometry_normal/normal.frag", 
        &display);
    // let program = create_program(
    ...
    event_loop.run(move |event, _, control_flow| {
        ...
        *control_flow = event_loop::ControlFlow::WaitUntil(next_frame_time);
        ...
        for model in models.iter() {
            ...
            target.draw(&model.vertex_buffer, &model.index_buffer, &normal_program, &uniforms, &draw_parameters).unwrap();
        }
